...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        ######################## Your code here #########################
        r_sum = R.sum(axis=0)
        return r_sum + self.prior.a_mix - 1
    ...
    def update_theta(self, X, R):
        ...
        ######################## Your code here #########################
        a = self.prior.a_pixels - 1
        b = self.prior.b_pixels - 1
        num = np.matmul(np.transpose(R), X) + a
        din = np.matmul(np.transpose(R), X) + a - b - (1 - X.sum())
        return num / din
    ...
    def compute_posterior(self, X, M=None):
        ...
        ######################## Your code here #########################
        log_p_z_x = np.dot(np.multiply(X, M), np.log(self.params.theta).T) + \
                          np.dot(1. - np.multiply(X, M), np.log(1. - self.params.theta).T) + np.log(self.params.pi)
    ...
    def posterior_predictive_means(self, X, M):
        ...
        ppm = np.matmul(np.transpose(R), X)
        return ppm
